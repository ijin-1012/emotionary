...
function openModal(data) {
  ...
  modalDiary.textContent = data.text;  // 일기 내용 표시
  
  // 모달 표시
  modal.style.display = "flex"; 
  console.log("모달이 열렸습니다.");  // 모달 열리는 로그
}

// === 모달 닫기 ===
closeModal.addEventListener("click", () => {
  modal.style.display = "none";  // 모달 닫기
  console.log("모달이 닫혔습니다.");  // 모달 닫히는 로그
});
...
modal.addEventListener("click", e => { 
  if (e.target === modal) {
    ...
    console.log("모달이 외부 클릭으로 닫혔습니다.");  // 외부 클릭으로 모달 닫히는 로그
  }
});
...
function renderCalendar() {
  ...
  for (let d = 1; d <= lastDate; d++) {
    ...
    cell.addEventListener("click", () => {
      ...
      console.log("모달을 여는 데이터:", data);  // 모달에 표시될 데이터 확인
      openModal({
        date: dateKey,
        text: data.text,
